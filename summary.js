...
function add2() {
  ...
  add2OB.innerHTML +=
    "Enthusiastic Job Title eager to contribute to team success through hard work, attention to detail and excellent organizational skills. Clear understanding of Task Job Title and training in Skill. Motivated to learn, grow and excel in Industry. ";
}
function add3() {
...
function bold() {
  ...
  document.getElementById("BlankSpaceForStrength").innerHTML = text.replace(
    highlight,
    span
  );
}
function italic() {
  let highlight = getSelectionText();
  let target = '<span>' + highlight + "</span>";
  if (target.style.fontStyle == "italic") {
    target.style.fontStyle = "none";
    let text = document.getElementById("BlankSpaceForStrength").innerHTML;
    document.getElementById("BlankSpaceForStrength").innerHTML = text.replace(highlight,target);
    console.log('none');
  } else {
    target.style.fontStyle = "italic";
    let text = document.getElementById("BlankSpaceForStrength").innerHTML;
...
const mementos = [];
const input = document.getElementById("BlankSpaceForStrength");

function saveMemento() {
  mementos.push(input.innerText);
}

function undo() {
  const lastMemento = mementos.pop();

  input.innerText = lastMemento;
  //   input.innerText = lastMemento ? lastMemento : input.innerText
}
